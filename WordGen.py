...
def addPhoneme(word):
    ...
    if is_onset:
        if cons_number < 2 and (last_son < 3 or last_son == 9):
        ...
        next_phon = random.randrange(0, len(l_freqs))
        new_phon = str(random.randrange(1, 9)) + l_freqs[next_phon][0] + '9'  # vowel sonority number
        word = word + new_phon
        cons_number = 0
        is_onset = False
...
# numbers before vowels indicate length and tone: 1 is flat, 2 rising, 3 falling, 4 rise-fall
# 5-8 correspond but are long vowels
